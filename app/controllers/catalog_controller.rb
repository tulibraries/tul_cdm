...
class CatalogController < ApplicationController
  ...
  configure_blacklight do |config|
    ...
    config.add_facet_field solr_name('digital_collection', :facetable), :label => 'Digital Collection', :limit => 5, :single => false, :collapse => false 
    config.add_facet_field solr_name('digital_publisher', :facetable), :label => 'Digital Publisher', :limit => 5
    config.add_facet_field solr_name('contentdm_collection_id', :facetable), :label => 'Collection ID', :limit => 5, :collapse => true, :show => false
    ...
    #   The ordering of the field names is the order of the display
    config.add_index_field solr_name('clip_title', :stored_searchable, type: :string), :label => 'Clip Title', :link_to_search => 'clip_title_sim'
