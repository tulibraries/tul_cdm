<?xml version="1.0" encoding="UTF-8"?>
  ...
  <xsl:template match="metadata/record">
    <xsl:copy>
      ...
      <xsl:variable name="rdfResource">
        <xsl:value-of select="concat('info:fedora/afmodel:', 'clipping')" />
      </xsl:variable>
          ...
          <foxml:datastreamVersion ID="descMetadata.0" LABEL="Descriptive metadata" MIMETYPE="text/xml">
            <foxml:xmlContent>
              <fields>
                ...
                <xsl:apply-templates select="Image_Number"/>
              </fields>
            </foxml:xmlContent>
          </foxml:datastreamVersion>
        </foxml:datastream>
  ...
  <xsl:template match="Image_Number">
    <image_number><xsl:apply-templates /></image_number>
  </xsl:template>
