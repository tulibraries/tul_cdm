<?xml version="1.0" encoding="UTF-8"?>
  ...
  <xsl:template match="metadata/record">
    <xsl:copy>
      ...
      <xsl:variable name="rdfResource">
        <xsl:value-of select="concat('info:fedora/afmodel:', 'video')" />
      </xsl:variable>
      <exsl:document method="xml" href="{$foxml_dir}/{$cdmfile}.xml">
        ...
        <foxml:datastream ID="descMetadata" STATE="A" CONTROL_GROUP="X" VERSIONABLE="true">
          <foxml:datastreamVersion ID="descMetadata.0" LABEL="Descriptive metadata" MIMETYPE="text/xml">
            <foxml:xmlContent>
              <fields>
                <xsl:apply-templates select="Clip_Title"/>
        ...
        <foxml:datastream ID="avMetadata" STATE="A" CONTROL_GROUP="X" VERSIONABLE="true">
          <foxml:datastreamVersion ID="avMetadata.0" LABEL="Audiovisual metadata" MIMETYPE="text/xml">
            <foxml:xmlContent>
              <fields>
                <xsl:apply-templates select="AVSOURCE"/>
                <xsl:apply-templates select="Clip_Summary"/>
                <xsl:apply-templates select="Date_Broadcast"/>
                <xsl:apply-templates select="Ensemble_Identifier"/>
                <xsl:apply-templates select="Timecode_Begin"/>
                <xsl:apply-templates select="Timecode_End"/>
                <xsl:apply-templates select="Transcript_Filename"/>
                <xsl:apply-templates select="Original_Source_Summary"/>
                <xsl:apply-templates select="Original_Source_Title"/>
              </fields>
            </foxml:xmlContent>
          </foxml:datastreamVersion>
        </foxml:datastream>
  ...
  <xsl:template match="Clip_Title">
    <clip_title><xsl:apply-templates /></clip_title>
  </xsl:template>
  ...
  <xsl:template match="AVSOURCE">
    <avsource><xsl:apply-templates /></avsource>
  </xsl:template>
  <xsl:template match="Clip_Summary">
    <clip_summary><xsl:apply-templates /></clip_summary>
  </xsl:template>
  <xsl:template match="Date_Broadcast">
    <date_broadcast><xsl:apply-templates /></date_broadcast>
  </xsl:template>
  <xsl:template match="Ensemble_Identifier">
    <ensemble_identifier><xsl:apply-templates /></ensemble_identifier>
  </xsl:template>
  <xsl:template match="Timecode_Begin">
    <timecode_begin><xsl:apply-templates /></timecode_begin>
  </xsl:template>
  <xsl:template match="Timecode_End">
    <timecode_end><xsl:apply-templates /></timecode_end>
  </xsl:template>
  <xsl:template match="Transcript_Filename">
    <transcript_filename><xsl:apply-templates /></transcript_filename>
  </xsl:template>
  <xsl:template match="Original_Source_Summary">
    <original_source_summary><xsl:apply-templates /></original_source_summary>
  </xsl:template>
