<?xml version="1.0" encoding="UTF-8"?>
      ...
      <exsl:document method="xml" href="{$foxml_dir}/{$cdmfile}.xml">
        ...
        <foxml:datastream ID="descMetadata" STATE="A" CONTROL_GROUP="X" VERSIONABLE="true">
          <foxml:datastreamVersion ID="descMetadata.0" LABEL="Descriptive metadata" MIMETYPE="text/xml">
            <foxml:xmlContent>
              <fields>
                <xsl:apply-templates select="Creator_Organization"/>
                <xsl:apply-templates select="Creator_Person"/>
                <xsl:apply-templates select="Other_Creator_Organization"/>
                <xsl:apply-templates select="Other_Creator_Person"/>
              </fields>
            </foxml:xmlContent>
          </foxml:datastreamVersion>
        </foxml:datastream>
  ...
  <xsl:template match="Creator_Organization">
    <creator_organization><xsl:apply-templates /></creator_organization>
  </xsl:template>
  <xsl:template match="Creator_Person">
    <creator_person><xsl:apply-templates /></creator_person>
  </xsl:template>
  <xsl:template match="Other_Creator_Organization">
    <other_creator_organization><xsl:apply-templates /></other_creator_organization>
  </xsl:template>
  <xsl:template match="Other_Creator_Person">
    <other_creator_person><xsl:apply-templates /></other_creator_person>
  </xsl:template>
