<?xml version="1.0" encoding="UTF-8"?>
  ...
  <xsl:template match="metadata/record">
    <xsl:copy>
      ...
      <xsl:variable name="rdfResource">
        <xsl:value-of select="concat('info:fedora/afmodel:', 'poster')" />
      </xsl:variable>
  ...
  <xsl:template match="Acknowledgment">
    <acknowledgment><xsl:apply-templates /></acknowledgment>
  </xsl:template>
  ...
  <xsl:template match="Hidden_Date">
    <hidden_date><xsl:apply-templates /></hidden_date>
  </xsl:template>
