<?xml version="1.0" encoding="UTF-8"?>
  ...
  <xsl:template match="metadata/record">
    <xsl:copy>
      ...
      <xsl:variable name="rdfResource">
        <xsl:value-of select="concat('info:fedora/afmodel:', 'pamphlet')" />
      </xsl:variable>
          ...
          <foxml:datastreamVersion ID="descMetadata.0" LABEL="Descriptive metadata" MIMETYPE="text/xml">
            <foxml:xmlContent>
              <fields>
                ...
                <xsl:apply-templates select="Internal_Note"/>
              </fields>
            </foxml:xmlContent>
          </foxml:datastreamVersion>
        </foxml:datastream>
  ...
  <xsl:template match="Date_of_Publication">
    <date_of_publication><xsl:apply-templates /></date_of_publication>
  </xsl:template>
  <xsl:template match="Internal_Note">
    <internal_note><xsl:apply-templates /></internal_note>
  </xsl:template>
