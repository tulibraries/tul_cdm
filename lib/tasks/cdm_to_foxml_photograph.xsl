<?xml version="1.0" encoding="UTF-8"?>
  ...
  <xsl:template match="metadata/record">
    <xsl:copy>
      ...
      <xsl:variable name="rdfResource">
        <xsl:value-of select="concat('info:fedora/afmodel:', 'photograph')" />
      </xsl:variable>
      <exsl:document method="xml" href="{$foxml_dir}/{$cdmfile}.xml">
        ...
        <foxml:datastream ID="descMetadata" STATE="A" CONTROL_GROUP="X" VERSIONABLE="true">
          <foxml:datastreamVersion ID="descMetadata.0" LABEL="Descriptive metadata" MIMETYPE="text/xml">
            <foxml:xmlContent>
              <fields>
                <xsl:apply-templates select="Intersection"/>
                <xsl:apply-templates select="Original_Notes"/>
  ...
  <xsl:template match="Intersection">
    <intersection><xsl:apply-templates /></intersection>
  </xsl:template>
  <xsl:template match="Original_Notes">
    <original_notes><xsl:apply-templates /></original_notes>
  </xsl:template>
  <xsl:template match="Photographer">
    <photographer><xsl:apply-templates /></photographer>
  </xsl:template>
