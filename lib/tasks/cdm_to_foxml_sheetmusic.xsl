<?xml version="1.0" encoding="UTF-8"?>
  ...
  <xsl:template match="metadata/record">
    <xsl:copy>
      ...
      <xsl:variable name="rdfResource">
        <xsl:value-of select="concat('info:fedora/afmodel:', 'sheetmusic')" />
      </xsl:variable>
      <exsl:document method="xml" href="{$foxml_dir}/{$cdmfile}.xml">
        ...
        <foxml:datastream ID="descMetadata" STATE="A" CONTROL_GROUP="X" VERSIONABLE="true">
          <foxml:datastreamVersion ID="descMetadata.0" LABEL="Descriptive metadata" MIMETYPE="text/xml">
            <foxml:xmlContent>
              <fields>
                <xsl:apply-templates select="Adapted_From"/>
                <xsl:apply-templates select="Alternate_Title"/>
                <xsl:apply-templates select="Contributor"/>
                <xsl:apply-templates select="Cover_Description"/>
                <xsl:apply-templates select="Date_Range"/>
                <xsl:apply-templates select="Display_Format"/>
                <xsl:apply-templates select="Donor_Information"/>
                <xsl:apply-templates select="Lithographer_Printer"/>
  ...
  <xsl:template match="Adapted_From">
    <adapted_from><xsl:apply-templates /></adapted_from>
  </xsl:template>
  ...
  <xsl:template match="Cover_Description">
    <cover_description><xsl:apply-templates /></cover_description>
  </xsl:template>
  <xsl:template match="Date_Range">
    <date_range><xsl:apply-templates /></date_range>
  </xsl:template>
  <xsl:template match="Display_Format">
    <display_format><xsl:apply-templates /></display_format>
  </xsl:template>
  <xsl:template match="Donor_Information">
    <donor_information><xsl:apply-templates /></donor_information>
  </xsl:template>
  <xsl:template match="Lithographer_Printer">
    <lithographer_printer><xsl:apply-templates /></lithographer_printer>
  </xsl:template>
